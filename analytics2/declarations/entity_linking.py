...
from declarations.entities import EntityInstance
...
class EntityLinker:
    ...
    def entities_from_text(self, text):
        ...
        except (http.client.RemoteDisconnected, http.client.CannotSendRequest) as err:
...
#                entity.entity = "NOT RECOGNISED"
    ...
    def enrich_email_bodies(self, corpus):
        for e in tqdm(corpus.iter_emails(), 
                      total=corpus.n_emails, desc="Enriching email bodies"):
            found_entities = self.entities_from_text(e.body)
            e.body.entities = found_entities
            for entity in e.body.entities:
                Universe.observe(entity, e.body, "mentioned_in")
